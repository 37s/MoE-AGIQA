...
class IQADataset(Dataset):
    ...
    def __init__(self, args, transform, status, loader=default_loader):
        ...
        self.txt_prompts = []
        self.scale = Info['subjective_scores_quality'][0, :].max()
        self.mos = Info['subjective_scores_quality'][0, self.index] / self.scale
        txt_prompts1 = [Info[Info['text_prompt1'][0, :][i]][()].tobytes()[::2].decode() for i in self.index]
        txt_prompts2 = [Info[Info['text_prompt2'][0, :][i]][()].tobytes()[::2].decode() for i in self.index]
        txt_prompts3 = [Info[Info['text_prompt3'][0, :][i]][()].tobytes()[::2].decode() for i in self.index]
        for i in range(len(txt_prompts1)):
            self.txt_prompts.append(txt_prompts1[i] + ", " + txt_prompts2[i] + ", " + txt_prompts3[i])
...
class IQARegression(nn.Module):
    def __init__(self, inchannels=768, outchannels=512, reduction=4):
        super().__init__()
        ...
        self.proj_prior = nn.Parameter(scale * torch.randn(inchannels, outchannels))

        self.fc = nn.Sequential(
            nn.Linear(outchannels, outchannels // reduction),
            nn.GELU(),
            nn.Linear(outchannels // reduction, outchannels),
            nn.Sigmoid()
        )
        self.weight1 = nn.Parameter(torch.zeros(1, outchannels, 14, 14))
        self.weight2 = nn.Parameter(torch.zeros(1, outchannels, 14, 14))
    ...
    def forward(self, x, text_features, vit_prior):
        ...
        vit_prior = vit_prior @ self.proj_prior
        vit_prior = self.fc(vit_prior).view(B, C, 1, 1)

        f_dis = (f_dis * self.weight1) + ((f_dis * vit_prior.expand_as(f_dis)) * self.weight2)
...
class Train:
    ...
    def create_model(self):
        ...
        self.vit_prior = timm.create_model("vit_base_patch16_224", pretrained=True).to(device)
        self.vit_prior.load_state_dict(torch.load(args.model_path, map_location=device)['vit_model_state_dict'])
    ...
    def train_epoch(self, epoch):
        ...
        for data in tqdm(self.train_loader):
            ...
            for i in range(self.opt.num_avg_train):
                ...
                vit_prior = self.vit_prior.forward_features(d_img_org)[:,0,:]
                ...
                pred += self.regressor(f_dis, text_features, vit_prior)
...
def parse_args():
    ...
    parser.add_argument('--name', type=str, default='aigciqa2023', \
